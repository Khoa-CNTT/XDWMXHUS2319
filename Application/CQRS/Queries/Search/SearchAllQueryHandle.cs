...
namespace Application.CQRS.Queries.Search
{
    public class SearchAllQueryHandle : IRequestHandler<SearchAllQuery, ResponseModel<List<SearchResultDto>>>
    {
        ...
        public async Task<ResponseModel<List<SearchResultDto>>> Handle(SearchAllQuery request, CancellationToken cancellationToken)
        {
            ...
            if (request.OnlyUsers == true)
            {
                ...
                userResults = users.Select(UserMapping.MapToUserDto).ToList();
            }
            else if (request.OnlyPosts == true)
            {
                ...
                postResults = posts.Select(PostMapping.MaptoPostDto).ToList();
            }
            else 
            {
